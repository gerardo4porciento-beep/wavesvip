...
export function DateCalendar({ capacity, date, onDateSelect }: DateCalendarProps) {
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div className="relative p-4 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl w-full max-w-sm mx-auto">
        <style jsx global>{`
          .rdp { --rdp-cell-size: 35px; --rdp-accent-color: #D4AF37; --rdp-background-color: #D4AF37; margin: 0; }
        ...
        <Calendar
          ...
          classNames={{
            ...
            caption_label: "text-luxury-gold font-display text-lg tracking-wide",
            nav: "space-x-1 flex items-center absolute w-full justify-between px-1",
            nav_button: "h-7 w-7 bg-black/20 text-white hover:bg-luxury-gold hover:text-black rounded-full transition-all border border-white/10 shadow-lg",
            nav_button_previous: "left-0",
            nav_button_next: "right-0",
            table: "w-full border-collapse",
            head_row: "flex justify-between mb-1",
            head_cell: "text-neutral-500 rounded-md w-8 font-normal text-[0.7rem] uppercase tracking-widest",
            row: "flex w-full mt-1 justify-between",
            cell: "text-center text-xs p-0 relative focus-within:relative focus-within:z-20",
            day: "h-8 w-8 p-0 font-medium aria-selected:opacity-100 rounded-full transition-all text-neutral-300",
        ...
        />
        {checking && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-10 rounded-2xl transition-all animate-in fade-in">
            <div className="flex flex-col items-center gap-2">
              <Loader2 className="w-8 h-8 animate-spin text-luxury-gold" />
              <span className="text-luxury-gold text-xs font-medium tracking-widest uppercase">Verificando...</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 flex flex-col items-center gap-1">
        {date ? (
          <div className="animate-in slide-in-from-bottom-2 fade-in duration-500 text-center">
            <span className="text-neutral-400 text-xs uppercase tracking-widest block">Reserva para el</span>
            <p className="text-lg text-white font-display">
              {date.toLocaleDateString("es-VE", { weekday: 'long', day: 'numeric', month: 'long' })}
            </p>
          </div>
        ) : (
          <p className="text-neutral-500 text-xs font-medium animate-pulse uppercase tracking-widest">
